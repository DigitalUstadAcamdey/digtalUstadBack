...
const normalizeChargilyMethod = (method) =>
...
exports.createPayment = catchAsync(async (req, res, next) => {
  ...
  await Transaction.create({
    ...
    status: "PENDING",
    paymentMethod: normalizeChargilyMethod(payment_method),
...
exports.addWebhook = catchAsync(async (req, res, next) => {
  ...
  if (computedSignature !== signature) {
    return next(
      new AppError("signature doesn't match the received signature", 403),
    );
  }
  ...
  switch (event.type) {
    case "checkout.paid":
      ...
      // Prevent duplicate balance updates for repeated webhook deliveries.
      ...
      transaction.paymentMethod =
        normalizeChargilyMethod(checkout.payment_method) ||
        transaction.paymentMethod ||
        null;
    ...
    case "checkout.failed":
      ...
      await Transaction.findOneAndUpdate(
        {
          userId: failedCheckout?.metadata?.userId,
          chargilyCheckoutId: failedCheckout?.id,
        },
        {
          status: "FAILED",
          metadata: {
